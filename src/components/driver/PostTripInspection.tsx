...
import { FlagOff, AlertTriangle, Loader2 } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';

interface PostTripInspectionProps {
  tripId: string;
  vehicleId?: string;
  vehiclePlate?: string;
  onSuccess?: () => void;
...
export function PostTripInspection({
  tripId,
  vehicleId,
  vehiclePlate,
  onSuccess,
}: PostTripInspectionProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log('Post-trip inspection:', {
        tripId,
        vehicleId,
        vehiclePlate,
        ...formData,
      ...
      toast({
        title: '✅ Báo cáo kết thúc xe',
        description: 'Kiểm tra xe kết thúc chuyến đã được lưu',
      });
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          <h3 className="font-medium text-sm">Kiểm tra tình trạng sau chuyến</h3>
        ...
        {/* Damages */}
        <div>
          <label className="text-sm">Hư hỏng / Trầy xước</label>
          <Textarea
            value={formData.damages}
            onChange={(e) =>
              setFormData({ ...formData, damages: e.target.value })
            }
            placeholder="Mô tả bất kỳ hư hỏng nào..."
            className="mt-1"
            rows={2}
          />
        </div>
