...
import { Filter, X, Search, ChevronDown, AlertTriangle } from 'lucide-react';
...
const STORAGE_KEY = 'vehicles_filters';

export function ExcelFilter({ data, filterConfigs, activeFilters, onFilterChange }: ExcelFilterProps) {
            ...
            for (const key of Object.keys(activeFilters)) {
                ...
                if (typeof filterValue === 'number') {
                    ...
                    if (config?.expiryField) {
                        const expiryDate = row[config.expiryField] as string;
                        if (expiryDate) {
                            const daysUntil = Math.ceil((new Date(expiryDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
                            if (daysUntil > filterValue || daysUntil < 0) return false;
                        } else {
                            return false;
                        }
                    }
            ...
            {activeFilterCount > 0 && (
                <>
                    ...
                    <span className="text-xs text-muted-foreground">
                        {filteredDataCount} / {data.length} xe
                    </span>
                </>
            )}
        </div>
