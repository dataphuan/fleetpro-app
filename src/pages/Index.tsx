...
import { Check, Truck, BarChart3, Shield, Users, MapPin, Zap, ChevronRight, Star, ArrowRight, Phone } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden">
      ...
      <section className="pt-32 pb-20 px-4 relative">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight mb-6 bg-gradient-to-r from-white via-white to-blue-200 bg-clip-text text-transparent">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="bg-blue-600 hover:bg-blue-500 text-lg h-14 px-8 shadow-lg shadow-blue-500/25" asChild>
              <Link to="/auth">
                Dùng thử miễn phí 14 ngày <ArrowRight className="w-5 h-5 ml-2" />
              </Link>
            </Button>
            <Button size="lg" variant="outline" className="border-slate-700 text-slate-300 hover:bg-slate-800 text-lg h-14 px-8" asChild>
              <a href="https://zalo.me/0838241234">
                <Phone className="w-5 h-5 mr-2" /> Tư vấn qua Zalo
              </a>
            </Button>
          </div>
          <p className="text-sm text-slate-500 mt-4">Không cần thẻ tín dụng • Thiết lập trong 5 phút • Hỗ trợ tiếng Việt</p>
        </div>
      </section>
      ...
      <section className="py-12 border-y border-white/5 bg-slate-900/50">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            <div key={label}>
              <div className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">{num}</div>
      ...
      <section id="features" className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">Tất cả trong một nền tảng</h2>
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">Không cần Excel, không cần sổ tay. Mọi thứ tự động hóa từ A đến Z.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <Card key={f.title} className="bg-slate-800/50 border-slate-700/50 hover:border-blue-500/30 transition-all duration-300 hover:-translate-y-1 group">
      ...
      <section className="py-20 px-4 bg-slate-900/50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {TESTIMONIALS.map((t) => (
              <Card key={t.name} className="bg-slate-800/50 border-slate-700/50">
                <CardContent className="p-6">
      ...
      <section id="pricing" className="py-20 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">Bảng giá đơn giản, minh bạch</h2>
            <p className="text-slate-400 text-lg">Chọn gói phù hợp với quy mô đội xe của bạn</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {PLANS.map((p) => (
              <Card key={p.name} className={`relative overflow-hidden transition-all duration-300 hover:-translate-y-2 ${p.highlight ? 'bg-gradient-to-b from-blue-600/20 to-slate-800/50 border-blue-500/50 shadow-xl shadow-blue-500/10' : 'bg-slate-800/50 border-slate-700/50'}`}>
                {p.highlight && (
                  <div className="absolute top-0 left-0 right-0 bg-blue-600 text-center text-xs font-bold py-1 text-white">PHỔ BIẾN NHẤT</div>
                )}
                <CardContent className={`p-6 ${p.highlight ? 'pt-10' : ''}`}>
                  ...
                  <Button className={`w-full ${p.highlight ? 'bg-blue-600 hover:bg-blue-500' : 'bg-slate-700 hover:bg-slate-600'}`} asChild>
                    <Link to="/auth">{p.cta} <ChevronRight className="w-4 h-4 ml-1" /></Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
      ...
      <section className="py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl font-bold mb-6">Sẵn sàng số hóa đội xe?</h2>
          <p className="text-slate-400 text-lg mb-8">Đăng ký dùng thử miễn phí ngay hôm nay. Không cần thẻ tín dụng, không ràng buộc.</p>
          <Button size="lg" className="bg-blue-600 hover:bg-blue-500 text-lg h-14 px-10 shadow-lg shadow-blue-500/25" asChild>
      ...
      <footer className="border-t border-white/5 py-12 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center font-bold text-sm">FP</div>
            <span className="font-bold">FleetPro</span>
            <span className="text-slate-500 text-sm ml-2">© 2026 TNC Solutions</span>
          </div>
          <div className="flex items-center gap-6 text-sm text-slate-400">
            <a href="mailto:support@tnc.io.vn" className="hover:text-white transition-colors">support@tnc.io.vn</a>
            <a href="tel:0838241234" className="hover:text-white transition-colors">083.824.1234</a>
            <a href="https://zalo.me/0838241234" className="hover:text-white transition-colors">Zalo</a>
          </div>
        </div>
      </footer>
    </div>
